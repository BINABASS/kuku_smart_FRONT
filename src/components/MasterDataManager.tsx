...
function MasterDataManager<T extends { id: number }>({ title, endpoint, columns, fields, normalizeOut, normalizeIn }: MasterDataManagerProps<T>) {
  ...
  const fetchItems = async () => {
    try {
      ...
      toast({ title: 'Error', description: 'Failed to fetch data', status: 'error', duration: 4000, isClosable: true });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Box w="100%">
      <VStack spacing={6} align="stretch" w="100%">
        ...
        <Card bg={cardBg} borderColor={borderColor} w="100%" minH={{ base: '50vh', md: '60vh' }}>
          <CardHeader>
