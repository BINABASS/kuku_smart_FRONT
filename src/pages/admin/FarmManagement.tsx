...
const FarmManagement = () => {
  ...
  useEffect(() => {
    let filtered = farms.filter(farm =>
      ...
      farm.farmer.user.first_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      farm.farmer.user.last_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      farm.city.toLowerCase().includes(searchTerm.toLowerCase()) ||
      farm.state.toLowerCase().includes(searchTerm.toLowerCase()) ||
      farm.country.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  const stats = {
    ...
    totalBatches: farms.reduce((sum, f) => sum + f.total_batches, 0),
    activeBatches: farms.reduce((sum, f) => sum + f.active_batches, 0),
    totalDevices: farms.reduce((sum, f) => sum + f.total_devices, 0),
    activeDevices: farms.reduce((sum, f) => sum + f.active_devices, 0),
    totalBirds: farms.reduce((sum, f) => sum + f.total_birds, 0),
    totalAcres: farms.reduce((sum, f) => sum + f.size_acres, 0),
  };
              ...
              <Tbody>
                {filteredFarms.map((farm) => (
                  <Tr key={farm.id}>
                    ...
                    <Td>
                      <VStack align="start" spacing={0}>
                        <Text fontSize="sm">{farm.city}, {farm.state}</Text>
                        <Text fontSize="xs" color={textColor}>{farm.country}</Text>
                        <Link
                    ...
                    <Td>
                      <VStack align="start" spacing={1}>
                        <Text fontSize="sm">{farm.size_acres} acres</Text>
                        <Badge colorScheme={getStatusColor(farm.status)}>
                          {farm.status}
                        </Badge>
                      </VStack>
                    </Td>
                    <Td>
                      <VStack align="start" spacing={0}>
                        <Text fontSize="sm">
                          <Text as="span" fontWeight="semibold">{farm.active_batches}</Text>/{farm.total_batches} batches
                        </Text>
                        <Text fontSize="sm">
                          <Text as="span" fontWeight="semibold">{farm.active_devices}</Text>/{farm.total_devices} devices
                        </Text>
                        <Text fontSize="xs" color={textColor}>
                          {farm.total_birds} birds
                        </Text>
                      </VStack>
                    </Td>
                    <Td>
                    ...
                    <Td>
                      <HStack spacing={2}>
                        ...
                        <IconButton
                          ...
                          icon={farm.status === 'ACTIVE' ? <CloseIcon /> : <CheckIcon />}
                          size="sm"
                          variant="ghost"
                          colorScheme={farm.status === 'ACTIVE' ? 'red' : 'green'}
